(function(){
  ...
  const statusLabel=s=>({new:'جديد',confirmed:'تم التأكيد',preparing:'جاري التجهيز',ready:'طلبكم جاهز',assigned:'تم التعيين',accepted:'تم القبول',picked_up:'تم الاستلام',out_for_delivery:'خرج للتوصيل',delivered:'تم التسليم',cancelled:'ملغي'})[s]||s||'—';
  ...
  function ready(){
    return typeof db!=='undefined'&&!!db&&typeof store!=='undefined'&&!!store?.restaurant?.id;
  }
  ...
  async function rpcOrders(token){
    if(!ready())return {error:new Error('قاعدة البيانات غير جاهزة'),data:[]};
    return await db.rpc('driver_get_orders',{p_token:token});
  }
  async function updateStatus(token,id,status){
    if(!ready())return {error:new Error('قاعدة البيانات غير جاهزة')};
    return await db.rpc('driver_update_status',{p_token:token,p_order_id:id,p_status:status});
  }
  ...
  function card(x){
    ...
    const lat=x.customer_lat??x.latitude,lng=x.customer_lng??x.longitude;
    const maps=lat!=null&&lng!=null?`<a target="_blank" rel="noopener noreferrer" href="https://www.google.com/maps?q=${Number(lat)},${Number(lng)}" class="inline-flex flex-1 items-center justify-center rounded-xl border px-3 py-3 font-bold">فتح الموقع</a>`:'';
  ...
  function driverIdFromOrders(orders){return String(orders[0]?.driver_id||orders[0]?.driverId||'');}
  ...
  async function load(){
    ...
    saveToken(token);
    app.innerHTML=`<main class="min-h-screen luxury-page p-4"><div class="max-w-3xl mx-auto pt-12"><div class="lux-card rounded-3xl p-6 text-center"><div class="text-4xl mb-3">🚚</div><h1 class="text-2xl font-extrabold">جاري فتح لوحة المندوب...</h1><div class="mt-3 text-sm" style="color:var(--muted)">جارٍ تجهيز الاتصال.</div></div></div></main>`;
    const ok=await waitReady();
    if(!ok){app.innerHTML=`<main class="min-h-screen luxury-page p-4"><div class="max-w-md mx-auto pt-16"><div class="lux-card rounded-3xl p-6 text-center"><h1 class="text-2xl font-extrabold">تعذر فتح صفحة المندوب</h1><p class="mt-3" style="color:var(--muted)">تعذر تجهيز الاتصال بقاعدة البيانات. أعد المحاولة بعد لحظات.</p></div></div></main>`;return;}
    const good=await refresh(false);
    if(!good){app.innerHTML=`<main class="min-h-screen luxury-page p-4"><div class="max-w-md mx-auto pt-16"><div class="lux-card rounded-3xl p-6 text-center"><h1 class="text-2xl font-extrabold">تعذر فتح صفحة المندوب</h1><p class="mt-3" style="color:var(--muted)">الرابط غير صالح أو لا يمكن الوصول للبيانات.</p></div></div></main>`;return;}
    setupRealtime();
