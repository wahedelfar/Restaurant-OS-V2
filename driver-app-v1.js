(function(){
  ...
  async function waitReady(){for(let i=0;i<180;i++){try{if(window.db&&window.store?.restaurant?.id)return true}catch(_){}await sleep(100)}return false}
  function setMenu(){try{history.pushState(null,'','#menu')}catch(_){location.hash='#menu'}if(typeof window.renderRouter==='function')setTimeout(()=>window.renderRouter(),0)}
  async function render(){
    const app=document.querySelector('#app');if(!app)return;const t=token();
  ...
  async function startGps(t,orderId,button){if(!navigator.geolocation){alert('المتصفح لا يدعم تحديد الموقع');return}button.disabled=true;button.textContent='GPS يعمل...';const send=p=>db.rpc('driver_update_location',{p_token:t,p_order_id:orderId,p_latitude:p.coords.latitude,p_longitude:p.coords.longitude,p_accuracy:p.coords.accuracy??null});const once=()=>navigator.geolocation.getCurrentPosition(async p=>{const r=await send(p);if(r.error)alert(r.error.message||'تعذر إرسال الموقع')},e=>alert(e.message||'تعذر تحديد الموقع'),{enableHighAccuracy:true,maximumAge:5000,timeout:20000});once();if(window.__rosDriverWatch)navigator.geolocation.clearWatch(window.__rosDriverWatch);window.__rosDriverWatch=navigator.geolocation.watchPosition(send,e=>console.warn('GPS',e),{enableHighAccuracy:true,maximumAge:5000,timeout:20000})}
  let rendering=false;async function route(){if(!isDriver()||rendering)return;rendering=true;try{await render()}finally{rendering=false}}
  window.addEventListener('hashchange',()=>{if(isDriver())route()},true);if(isDriver())setTimeout(route,0);
})();
