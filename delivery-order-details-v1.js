...
async function customer(){
  ...
  const t=decodeURIComponent((location.hash||'').slice(7));if(!t||!window.db)return;token=t;
  try{const r=await window.db.rpc('public_track_order_v3',{p_token:t});if(r.error||!r.data?.length)return;const x=r.data[0],box=document.querySelector('#rosTrackBox');if(!box)return;box.querySelectorAll('[data-ros-order-details="customer"]').forEach(el=>el.remove());box.insertAdjacentHTML('afterbegin',card(x,'customer'))}catch(_){ }
}
async function driver(){
  if(!/^#driver\//.test(location.hash||''))return;
  const t=decodeURIComponent((location.hash||'').slice(8));if(!t||!window.db)return;token=t;
