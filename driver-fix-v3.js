(function(){
'use strict';
if(window.__ROS_DRIVER_FIX_V4__) return;
window.__ROS_DRIVER_FIX_V4__=true;
...
function setMenu(){location.hash='menu';if(typeof window.renderRouter==='function')setTimeout(()=>window.renderRouter(),0)}
async function render(){
 ...
 box.querySelectorAll('.driver-status').forEach(b=>b.addEventListener('click',async()=>{b.disabled=true;const x=await db.rpc('driver_update_status',{p_token:t,p_order_id:b.dataset.id,p_status:b.dataset.status});if(x.error)alert(x.error.message||'تعذر تحديث الحالة');else render()}));
 box.querySelectorAll('.driver-gps').forEach(b=>b.addEventListener('click',()=>{b.disabled=true;b.textContent='GPS يعمل...';const send=p=>db.rpc('driver_update_location',{p_token:t,p_order_id:b.dataset.gps,p_latitude:p.coords.latitude,p_longitude:p.coords.longitude,p_accuracy:p.coords.accuracy??null});navigator.geolocation.getCurrentPosition(send,e=>alert(e.message||'تعذر تحديد الموقع'),{enableHighAccuracy:true,maximumAge:5000,timeout:20000});if(window.__rosDriverWatch)navigator.geolocation.clearWatch(window.__rosDriverWatch);window.__rosDriverWatch=navigator.geolocation.watchPosition(send,e=>console.warn('GPS',e),{enableHighAccuracy:true,maximumAge:5000,timeout:20000})}));
}
if(isDriver())setTimeout(render,1000);
window.addEventListener('hashchange',()=>{if(isDriver())setTimeout(render,300)},true);
})();
