(function(){
  ...
  function statusLabel(s){
    return ({
      new:'جديد',confirmed:'تم التأكيد',preparing:'قيد التحضير',ready:'جاهز',
      assigned:'تم التعيين',out_for_delivery:'خرج للتوصيل',delivered:'تم التسليم',
      cancelled:'ملغي',completed:'مكتمل',unassigned:'بدون مندوب',accepted:'تم القبول',
  ...
  window.sendDeliveryOrder=async function(){
    ...
    let proofUrl=null;
    try{
      if(pay==='vodafone')proofUrl=await uploadProof(proofFile);
    }catch(e){return notify(e?.message||'تعذر رفع صورة التحويل');}
  ...
  window.assignDriver=async function(orderId){
    ...
    await refreshDeliveryPanel();
    notify('تم تعيين المندوب');
  };

  async function renderAdminWithDelivery(){
    try{await originalRenderAdmin();}catch(e){console.error(e);}
    const app=$('#app');
