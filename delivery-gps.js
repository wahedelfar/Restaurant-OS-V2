...
(function(){
  ...
  function startGps(token,orderId){
    ...
    if(!window.isSecureContext)return toast('GPS يحتاج HTTPS — افتح رابط Vercel');
    gps.watchId=navigator.geolocation.watchPosition(async p=>{
      gps.last=p.coords;
      try{await db.rpc('driver_update_location',{p_token:token,p_order_id:orderId,p_latitude:p.coords.latitude,p_longitude:p.coords.longitude,p_accuracy:p.coords.accuracy||null})}catch(e){console.warn('GPS update failed',e)}
  ...
  async function createDriver(){
    const name=$('#newDriverName')?.value.trim(),phone=$('#newDriverPhone')?.value.trim();if(!name)return toast('اكتب اسم المندوب');
    const access_token=crypto.randomUUID();
    const r=await db.from('drivers').insert({restaurant_id:store.restaurant.id,name,phone:phone||null,access_token,active:true}).select().single();if(r.error)return toast(r.error.message);
  ...
  async function assignOrder(orderId){
    const driverId=$(`#driver_${orderId}`)?.value;if(!driverId)return toast('اختر مندوبًا');
    const r=await db.from('delivery_orders').upsert({order_id:orderId,restaurant_id:store.restaurant.id,driver_id:driverId,status:'assigned'},{onConflict:'order_id'});if(r.error)return toast(r.error.message);
    const u=await db.from('orders').update({status:'assigned'}).eq('id',orderId).eq('restaurant_id',store.restaurant.id);if(u.error)console.warn(u.error);
    toast('تم تعيين المندوب');await loadSupabase();await renderAdmin();
  }
  window.assignOrder=assignOrder;
  async function adminOrderStatus(orderId,status){const r=await db.from('orders').update({status}).eq('id',orderId).eq('restaurant_id',store.restaurant.id);if(r.error)return toast(r.error.message);await loadSupabase();await renderAdmin()}
  ...
  async function renderDeliveryAdmin(){
    ...
    const drivers=driversQ.data||[],dels=delQ.data||[],deliveryOrders=store.orders.filter(o=>o.order_type==='delivery');
    const sec=document.getElementById('deliverySection');if(!sec)return;
    const driversHtml=drivers.length?drivers.map(d=>{const link=location.origin+location.pathname+'#driver/'+encodeURIComponent(d.access_token);return `<div class="border rounded-2xl p-4 flex justify-between gap-3 flex-wrap"><div><b>${esc2(d.name)}</b><div class="text-sm text-gray-500">${esc2(d.phone||'')} • ${d.active?'نشط':'متوقف'}</div></div><button type="button" onclick="navigator.clipboard?.writeText('${link.replace(/'/g,"\\'")}');toast('تم نسخ رابط المندوب')" class="px-3 py-2 rounded-xl border font-bold">نسخ رابط المندوب</button></div>`}).join(''):'<div class="text-gray-500">لا يوجد مندوبون بعد.</div>';
    const ordersHtml=deliveryOrders.length?deliveryOrders.map(o=>{const d=dels.find(x=>x.order_id===o.id);const driverOptions=drivers.filter(x=>x.active).map(x=>`<option value="${x.id}" ${d?.driver_id===x.id?'selected':''}>${esc2(x.name)}</option>`).join('');const tracking=o.tracking_token?btn('تتبع العميل',`location.hash='track/${o.tracking_token}'`,'border bg-white text-black'):'';return `<div class="border rounded-2xl p-4"><div class="flex justify-between gap-3 flex-wrap"><div><b>طلب #${esc2(o.id?.slice?.(0,8)||'')}</b> — ${esc2(o.customer_name||'عميل')}</div><b>${fmtMoney(o.total)}</b></div><div class="text-sm text-gray-500 mt-2">${esc2(o.customer_phone||'')} • ${esc2(o.address||'بدون عنوان')}</div><div class="flex flex-wrap gap-2 mt-3"><select id="driver_${o.id}" class="border rounded-xl p-3"><option value="">اختر المندوب</option>${driverOptions}</select>${btn('تعيين',`assignOrder('${o.id}')`)}<select onchange="adminOrderStatus('${o.id}',this.value)" class="border rounded-xl p-3"><option value="">حالة الطلب</option>${Object.entries(statusNames).map(([k,v])=>`<option value="${k}" ${o.status===k?'selected':''}>${v}</option>`).join('')}</select>${tracking}</div><div class="text-xs text-gray-500 mt-2">${d?`المندوب: ${esc2(drivers.find(x=>x.id===d.driver_id)?.name||'')} • ${driverStatusNames[d.status]||d.status}`:'لم يتم تعيين مندوب'}</div></div>`}).join(''):'<div class="text-gray-500">لا توجد طلبات توصيل.</div>';
    sec.innerHTML=card(`<div class="flex items-center justify-between gap-3 flex-wrap"><div><h2 class="text-xl font-extrabold">إدارة التوصيل + GPS</h2><p class="text-sm text-gray-500 mt-1">إنشاء مندوب، تعيين الطلب، ومتابعة موقعه لحظيًا.</p></div><span class="px-3 py-2 rounded-full bg-gray-100">${deliveryOrders.length} طلب توصيل</span></div><div class="grid md:grid-cols-3 gap-3 mt-4"><input id="newDriverName" class="border rounded-2xl p-4" placeholder="اسم المندوب"><input id="newDriverPhone" class="border rounded-2xl p-4" placeholder="رقم المندوب"><div>${btn('إضافة مندوب','createDriver()')}</div></div><div id="driverCreateResult"></div><div class="mt-5 space-y-3">${driversHtml}</div><div class="mt-5"><h3 class="font-extrabold text-lg mb-3">طلبات التوصيل</h3><div class="space-y-3">${ordersHtml}</div></div>`);
  }
  const originalCheckout=window.checkout;
  window.checkout=function(){
    const table=tableFromUrl();if(table)return originalCheckout();
  ...
  window.getCustomerLocation=function(){
    if(!navigator.geolocation)return toast('المتصفح لا يدعم GPS');if(!window.isSecureContext)return toast('GPS يحتاج HTTPS');
  ...
  window.sendDeliveryOrder=async function(){
    ...
    const name=$('#cust')?.value.trim()||'عميل',phone=$('#customerPhone')?.value.trim()||'',address=$('#addr')?.value.trim()||'',pay=$('#pay')?.value||'cash';if(!name||!phone||!address)return toast('اكتب الاسم ورقم الهاتف والعنوان');
    const total=cart.reduce((a,b)=>a+b.price*b.qty,0),fallbackTracking=crypto.randomUUID(),items=cart.map(x=>({product_id:x.id,name:x.name,quantity:x.qty,price:x.price}));
    const r=await db.rpc('create_delivery_order',{p_restaurant_id:store.restaurant.id,p_customer_name:name,p_customer_phone:phone,p_address:address,p_payment_method:pay,p_items:items,p_customer_lat:customerCoords?.lat||null,p_customer_lng:customerCoords?.lng||null});if(r.error)return toast(r.error.message||'تعذر إنشاء الطلب');
    const result=r.data?.[0]||r.data,tracking=result?.tracking_token||fallbackTracking,trackUrl=location.origin+location.pathname+'#track/'+tracking;
    const msg=`طلب توصيل جديد من ${store.restaurant.name}\nالاسم: ${name}\nالهاتف: ${phone}\nالعنوان: ${address}\n\n${cart.map(x=>`${x.name} × ${x.qty}`).join('\n')}\n\nالإجمالي: ${fmtMoney(total)}\nتتبع الطلب: ${trackUrl}`;cart=[];updateCart();closeModal();const wa=getWaNumber();window.location.href=`https://wa.me/${wa}?text=${encodeURIComponent(msg)}`;
  };
  ...
  async function renderDriver(token){
    stopGps();if(window.__trackTimer)clearInterval(window.__trackTimer);
  ...
  window.renderRouter=function(){const h=location.hash||'#menu';if(h.startsWith('#track/'))return renderTrack(tokenFromHash('track'));if(h.startsWith('#driver/'))return renderDriver(tokenFromHash('driver'));if(window.__trackTimer){clearInterval(window.__trackTimer);window.__trackTimer=null}if(window.__driverTimer){clearInterval(window.__driverTimer);window.__driverTimer=null}stopGps();return baseRouter()};
  window.addEventListener('hashchange',window.renderRouter);
  const oldAdmin=window.renderAdmin;
  window.renderAdmin=async function(){await oldAdmin();if(!document.getElementById('deliverySection')){const main=document.querySelector('#app main');if(main){const wrap=document.createElement('section');wrap.id='deliverySection';wrap.className='mt-5';main.appendChild(wrap)}}await renderDeliveryAdmin()};
