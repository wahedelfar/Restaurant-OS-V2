(function(){
  ...
  function adminCard(offer){
    ...
    host.innerHTML=`<div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap"><div><div style="font-size:21px;font-weight:900">عرض اليوم</div><div style="font-size:12px;color:var(--muted,#A9A39A);margin-top:4px">اختر منتجًا أو أكثر واكتب نص العرض الذي سيظهر للعميل مرة واحدة عند فتح الموقع.</div></div><div style="display:flex;align-items:center;gap:10px"><span id="ros-daily-offer-status"></span><button id="ros-daily-offer-close" type="button" aria-label="إغلاق عرض اليوم" title="إغلاق" style="width:38px;height:38px;border-radius:50%;border:1px solid #ffffff22;background:#ffffff08;color:var(--text,#F6F1E7);font-size:22px;font-weight:900;cursor:pointer;line-height:1">×</button></div></div><div style="margin-top:15px"><label style="display:block;font-weight:800;margin-bottom:7px">المنتجات</label><select id="ros-daily-offer-products" multiple size="7"></select></div><div style="margin-top:13px"><label style="display:block;font-weight:800;margin-bottom:7px">نص العرض</label><textarea id="ros-daily-offer-text" rows="3" placeholder="مثال: خصم 50% أو اشتري واحدة واحصل على الأخرى بنصف الثمن"></textarea></div><div class="ros-offer-actions"><button id="ros-daily-offer-save" type="button">حفظ عرض اليوم</button><button id="ros-daily-offer-disable" type="button">إخفاء العرض</button></div>`;
    ...
    const status=host.querySelector('#ros-daily-offer-status');status.textContent=offer?.active?'العرض ظاهر للعملاء':'العرض مخفي';host.querySelector('#ros-daily-offer-close').addEventListener('click',e=>{e.preventDefault();e.stopPropagation();host.remove();});
