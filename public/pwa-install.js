...
function restoreIcons(){
  ...
  const icon=document.createElement('link');icon.rel='icon';icon.type='image/svg+xml';icon.href='/icon.svg';document.head.appendChild(icon);
  const apple=document.createElement('link');apple.rel='apple-touch-icon';apple.href='/icon.svg';document.head.appendChild(apple);
}
...
function make(){if(button)return;button=document.createElement('button');button.id='pwa-install';button.type='button';button.title=t('install');button.setAttribute('aria-label',t('install'));button.innerHTML='<img src="/icon.svg" alt="">';button.onclick=install;document.body.appendChild(button)}
