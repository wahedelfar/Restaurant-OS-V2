...
export default function MenuClient({ products, categories, subcategories, modifiers }: Props) {
  ...
  function addToCart() {
    ...
    setCart(prev => {
      ...
      return found ? prev.map(x => x.key === key ? { ...x, qty: x.qty + 1 } : x) : [...prev, { key, id: selected.id, name: selected.name, size: size.size, price, qty: 1, modifiers: selectedMods }]
    })
    setModal(null)
  }

  function change(key: string, delta: number) {
    setCart(prev => prev.flatMap(x => x.key === key ? (x.qty + delta > 0 ? [{ ...x, qty: x.qty + delta }] : []) : [x]))
  }
  ...
  async function submitOrder() {
    ...
    try {
      const res = await fetch('/api/restaurant/order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        ...
        p_items: cart.map(x => ({ product_id: x.id, name: x.name, quantity: x.qty, price: x.price, size: x.size, modifiers: x.modifiers.map(m => ({ id: m.id, name: m.name, price: m.price })) }))
      }) })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'تعذر إرسال الطلب')
      const row = Array.isArray(data) ? data[0] : data
      setOrderToken(row?.tracking_token ?? '')
      setCart([])
      setModal('success')
    } catch (e) {
