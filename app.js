...
function productForm(id){
  ...
  $('#modal').innerHTML=`<div class="fixed inset-0 modal z-50 p-4 grid place-items-center" onclick="if(event.target===this)closeModal()"><div class="relative bg-white rounded-3xl p-5 w-full max-w-lg max-h-[92vh] overflow-auto"><button onclick="closeModal()" class="absolute top-4 left-4 w-10 h-10 rounded-full border text-2xl leading-none" aria-label="إغلاق">×</button><h2 class="text-2xl font-extrabold mb-4 pr-12">${id?'تعديل المنتج':'إضافة منتج'}</h2><div class="space-y-3"><input id="pn" value="${esc(p.name)}" class="w-full border rounded-2xl p-4" placeholder="اسم المنتج"><textarea id="pd" class="w-full border rounded-2xl p-4" placeholder="الوصف">${esc(p.description)}</textarea><input id="pp" type="number" min="0" step="1" value="${p.price}" class="w-full border rounded-2xl p-4" placeholder="السعر"><input id="pi" value="${esc(p.image_url||'')}" class="w-full border rounded-2xl p-4" placeholder="رابط الصورة"><select id="pc" class="w-full border rounded-2xl p-4">${store.categories.map(c=>`<option value="${c.id}" ${c.id===p.category_id?'selected':''}>${esc(c.name)}</option>`).join('')}</select><label class="flex gap-2"><input id="pa" type="checkbox" ${p.available?'checked':''}> متاح للبيع</label></div><button type="button" id="saveProductBtn" data-product-id="${id?esc(id):''}" class="w-full mt-5 py-4 rounded-2xl bg-black text-white font-bold">حفظ التعديل</button></div></div>`
}
