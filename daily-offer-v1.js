(function(){
  ...
  function style(){
    ...
    s.textContent=`
      #ros-daily-offer-modal{position:fixed;inset:0;z-index:9999;display:none;align-items:center;justify-content:center;padding:18px;background:#000b;backdrop-filter:blur(10px)}
  ...
  function getTodayKey(){
    ...
    return `daily-offer-seen-${yyyy}-${mm}-${dd}`;
  }

  function showCustomerOffer(offer){
    if(isAdmin||!offer?.active||!String(offer.offer_text||'').trim())return;
    const todayKey=getTodayKey();
    if(localStorage.getItem(todayKey)==='1'||sessionStorage.getItem('ros_daily_offer_seen_v1')==='1')return;
